...
<body class="bg-slate-950 cabin h-screen text-white bg-linear-to-b from-black via-slate-950 to-slate-950 flex flex-col">
    ...
    <header>
        <nav class="p-5 flex items-center justify-between">
            <div class="flex items-baseline gap-6">
                <a href="/" class="text-3xl">Minnow</a>
    ...
    <main class="flex-1 flex flex-col items-center px-15 py-4">
        <h2 class="text-5xl font-bold hover:scale-110 duration-800 transition-transform">Welcome to Minnow.</h2>
        <h3 class="text-4xl tracking-wide mt-6">This is <span class="italic">your</span> media.</h3>
    
        <a href="/library" class="mt-25 text-2xl px-6 py-3 bg-slate-950 hover:bg-slate-900 border-2 border-slate-900 rounded-lg hover:scale-110 duration-400 transition">Open Library</a>

        <h4 class="text-3xl mt-20">Recently Added</h4>

        <div class="grid grid-cols-5 gap-8 w-full mt-5 max-w-5xl">
            <?php if (!empty($recent_media)): ?>
                <?php foreach ($recent_media as $media_file): 
